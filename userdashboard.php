...
$stmt = $conn->prepare("SELECT COUNT(*) FROM reservation WHERE user_id = ? ANd status != 'Cancelled'");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$reservation_count = $stmt->get_result()->fetch_row()[0];
$stmt->close();

// Count catering reservations
$stmt = $conn->prepare("SELECT COUNT(*) FROM reservation_catering WHERE user_id = ? and status != 'Cancelled'");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$catering_count = $stmt->get_result()->fetch_row()[0];
$stmt->close();

// Calculate total orders
$total_orders = $reservation_count + $catering_count;
// Fetch the count of pending reservations
$stmt = $conn->prepare("SELECT COUNT(*) FROM reservation WHERE user_id = ? AND status = 'Pending'");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$pending_reservations = $stmt->get_result()->fetch_row()[0];

$stmt = $conn->prepare("SELECT COUNT(*) FROM reservation_catering WHERE user_id = ? AND status = 'Pending'");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$pending_catering = $stmt->get_result()->fetch_row()[0];
...
$stmt->bind_param("i", $user_id);
$stmt->execute();
$upcoming_catering = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
