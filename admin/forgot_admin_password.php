...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['email'])) {
        ...
        if ($email) {
            include 'conn.php'; // Reconnect to the database
...
<!-- Header -->
<header class="bg-maroon-900 text-white py-4 sticky top-0 z-50">
    ...
    <section class="max-w-md mx-auto mt-16 bg-white p-8 rounded-lg shadow-lg">
        <?php if ($message): ?>
            <div class="text-center mb-4 text-maroon-700 font-semibold">
        ...
        <?php if ($step === 1): ?>
            ...
            <form method="POST" action="" class="fade-in">
                ...
                <button type="submit" style="background: linear-gradient(135deg, #800000, #4a0000);"
                    class="w-full text-white p-3 rounded-md hover:bg-maroon-600 focus:outline-none focus:ring-2 focus:ring-maroon-500">
                    Send PIN
                </button>
            </form>
        <?php elseif ($step === 2): ?>
