<div id="cancelled" class="tab-content pt-4 hidden">
    <div class="card-container">
        <?php foreach ($reservations as $reservation): ?>
            <?php if (strtolower($reservation['status']) === 'pending cancellation' || strtolower($reservation['status']) === 'cancelled'): ?>
                <div class="card" data-bs-toggle="modal"
                    data-bs-target="#cancelledReservationModal<?php echo $reservation['reservation_id']; ?>">
                    ...
                    <div class="card-body">
                        ...
                        <button type="button" class="btn btn-danger mt-2 p-1" data-bs-toggle="modal"
                            data-bs-target="#paymentcancelModal<?php echo $reservation['reservation_id']; ?>">
                ...
                <!-- Payment Modal -->
                <div class="modal fade" id="paymentcancelModal<?php echo $reservation['reservation_id']; ?>" tabindex="-1"
                ...
                <!-- Modal -->
                <div class="modal fade" id="cancelledReservationModal<?php echo $reservation['reservation_id']; ?>"
                    tabindex="-1" aria-labelledby="cancelledReservationModalLabel<?php echo $reservation['reservation_id']; ?>"
                    aria-hidden="true">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title"
                                    id="cancelledReservationModalLabel<?php echo $reservation['reservation_id']; ?>">Reservation
                                    Details</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            ...
                            <p class="mr-9"><strong>Total:</strong> ₱<?php echo number_format($total_price, 2); ?></p>
                                <div class="reservation-actions">
                                    <?php if (strtolower($reservation['status']) !== 'cancelled'): ?>
                                        <a href="manage_reservation.php?approve_cancel=<?php echo $reservation['reservation_id']; ?>#cancelled"
                                            class="btn btn-success btn-sm reservation-btn">
                                            <i class="fas fa-check"></i> Approve Cancellation
                                        </a>

                                        <a href="manage_reservation.php?reject_cancel=<?php echo $reservation['reservation_id']; ?>"
                                            class="btn btn-danger btn-sm reservation-btn">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Check if URL contains #cancelled
        if (window.location.hash === '#cancelled') {
            ...
            // If using Bootstrap tabs, activate the correct tab
            const cancelledTab = document.querySelector('[data-bs-target="#cancelled"]');
            if (cancelledTab) {
                new bootstrap.Tab(cancelledTab).show();
            }
        }
    });
</script>